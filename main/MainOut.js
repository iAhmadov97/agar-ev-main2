/**
 * mainout of agarev made with cairbyte71
 * @param { Window } wHandle global window with more handlers
 * @param {*} wjQuery jquery
 * @param {*} loaded loaded callback
 */

export const MainOut = (wHandle, wjQuery, loaded) => {
  try {
    ...
    function wsCleanup(onlyWs = false) {
      if (!ws) return;
      ...
      if (!onlyWs && wsMulti) {
        wsMulti.onopen = null;
        wsMulti.onclose = null;
        wsMulti.onmessage = null;
        wsMulti.close();
        wsMulti = null;
      }
    }

    function wsInit(url, onlyWs = false) {
      if (ws) {
      ...
      ws.onerror = (e) => console.log("ws1 error -> ", e);
      ws.onclose = wsClose;

      if (!onlyWs) {
        wsMulti = null;
        wsMulti = new WebSocket(`ws${USE_HTTPS ? "s" : ""}://${url}`);
        wsMulti.binaryType = "arraybuffer";
        wsMulti.onopen = () => {
          wsSend(window.wsStuffs.send_1, true);
          wsSend(window.wsStuffs.send_2, true);
        };
        wsMulti.onmessage = (data) => {
          ...
          switch (packetId) {
            case 0x10: // update nodes
              ...
              for (var i = 0; i < count; i++) {
                ...
                if (!cells2.byId.hasOwnProperty(killer) || !cells2.byId.hasOwnProperty(killed)) continue;
            ...
            case 0x11: // update pos
              if (wHandle.globalPreference.TAB_ACTIVE) {
        ...
        wsMulti.onerror = (e) => console.log("ws2 error -> ",e);
        wsMulti.onclose = (e) => {
          console.log("ws2 diconnected -> " + e.code + " " + e.reason)
          wHandle.globalPreference.TAB_CELL_INGAME = false;
          wsMulti.onopen = null;
          wsMulti.onclose = null;
          wsMulti.onmessage = null;
          wsMulti.close();
          wsMulti = null;
          setTimeout(() => {
            if (wsMulti && wsMulti.readyState === 1) return;
            wsInit(wsUrl, true);
          }, (disconnectDelay *= 1.5));
        };
      }
    ...
    function wsOpen() {
      ...
      wsSend(window.wsStuffs.send_1);
      wsSend(window.wsStuffs.send_2);
    ...
    function wsSend(data, isWS2) {
      let wsR = ws;
      if (isWS2 !== null) {
        if ((isWS2 && isWS2 === true) || wHandle.globalPreference.TAB_ACTIVE === true) {
          wsR = wsMulti;
        }
      }
      
      if (!wsR || wsR.readyState !== 1) return;
      if (data.build) wsR.send(data.build());
      else wsR.send(data);
    }
    wHandle.sendPacket = wsSend;
      ...
      switch (packetId) {
        ...
        case 0x40: // set border
          ...
          stats.pingLoopId = setInterval(() => {
            wsSend(window.wsStuffs.cache[254], false);
        ...
        case 0x63: // chat message
          ...
          if (accountDetails && "pid" in accountDetails) {
            ...
            chat.messages[idElement] = {
              ...
              tag: accountDetails && "tag" in accountDetails ? accountDetails.tag : null,
              server: server,
              admin: admin,
              mod: mod,
    ...
    function sendMouseMove(x, y, isWS2) {
      ...
      wsSend(writer, wHandle.globalPreference.TAB_ACTIVE === true ? true : false);
    }
    ...
    function sendPlay(name) {
      ...
      if (wHandle.globalPreference.TAB_ACTIVE === true) wHandle.globalPreference.TAB_ACTIVE = false;
      var writer = new Writer(true),
        skin = null, multiSkin = null;
      ...
      if (globalPreference.currentMulti) {
        multiSkin = globalPreference.currentMulti;
      }
      ...
      if (wHandle.account && typeof wHandle.account === "object") {
        if (wHandle.account.pid) wHandle.globalPreference.PID_PLAYER = wHandle.account.pid;
        let jsonEncrypted = null;
        if ("jsonEncrypted" in wHandle.account && wHandle.account.jsonEncrypted) {
          jsonEncrypted = wHandle.account.jsonEncrypted;
        }
        name = `${name}[${jsonEncrypted}]`;
      }

      wHandle.globalPreference = {
        ...wHandle.globalPreference,
        PLAYER_NAME: name,
        PLAYER_INGAME: true,
      };
    ...
    wHandle.pushIntoChatList = (object, draw) => {
      let idElement = Date.now().toString(36);
      let tempObject = {
        idElement: idElement,
    ...
    wHandle.sendSystemMessage = (message) => {
      wHandle.pushIntoChatList(
        {
          role: "BOT",
          verified: true,
          name: "Lyder Bot",
    ...
    function sendChat(text) {
      ...
      if (text.charAt(0) === "$" && wHandle.account) {
        ...
        if (command && typeof command === "string" && "1" in commandFullLine) {
          ...
          switch (command) {
            ...
            case "kill":
              ...
              if ("role" in wHandle.account && wHandle.wsServer) {
                if (!token) {
                  wHandle.onbeforeunload = null;
                  location.reload();
                  return;
                }

                if (
                  ("admin" in wHandle.account.role && wHandle.account.role.admin === true) ||
                  ("permission" in wHandle.account.role &&
                    command in wHandle.account.role.permission &&
                    wHandle.account.role.permission[command] === true)
                ) {
                  let writer = new Writer(),
                    packetId = command === "mute" ? 0x3 : 0x4;
                  ...
                  wHandle.wsServer.sendPacket(writer);
                } else {
                  wHandle.sendSystemMessage("ليس لديك صلحيه لهذا");
                }
              } else wHandle.sendSystemMessage("You can't do that right now maybe later");
              break;
          }
        } else wHandle.sendSystemMessage("Invalid command pleas check the list help");
      } else {
    ...
    wHandle.ret = sendChat;

    function gameReset(onlyMeine = false) {
      cleanupObject(cells);
    ...
    wHandle.hideESCOverlay = () => {
      escOverlayShown = false;
      wjQuery("#overlays").hide();
    }

    wHandle.showESCOverlay = (losed) => {
      if (losed === true) wHandle.globalPreference.PLAYER_INGAME = false;
          ...
          <div class="message" id="${elemntObject.idElement}">
            ...
            ${
              elemntObject.role.length > 0 || elemntObject.server
                ? `<div class="role${elemntObject.role.toLowerCase() === "bot" ? " rainbow":""}" style="color:${elemntObject.color}">${elemntObject.role || "server"}</div>`
                : ""
            }
            ${elemntObject.tag ? `<div class="id rainbow">${elemntObject.tag}</div>` : ""}
    ...
    function drawBorders() {
      ...
      let url = null;
      try {
        url = wHandle.globalPreference.SETTINGS.GENERALE?.content.BG_BORDER.url || "https://i.pinimg.com/originals/ac/74/99/ac74999a0c3ac959416fff86a02928bf.jpg";
      } catch (e) {}
    ...
    function drawMinimap() {
      ...
      mainCtx.beginPath();
      mainCtx.arc(myPosX, myPosY, 5, 0, window.wsStuffs.PI, false);
      mainCtx.closePath();
      mainCtx.fill();

      if (!wHandle.globalPreference.TAB_ACTIVE && cells2.mine.length) {
        let mineId = cells2.mine[cells2.mine.length - 1];
        if (mineId && mineId in cells2.byId) {
          let objectCell = cells2.byId[mineId];
          if (objectCell) {
            let myPosX2 = beginX + ((objectCell.x + border.width / 2) / border.width) * width;
            let myPosY2 = beginY + ((objectCell.y + border.height / 2) / border.height) * height;
  
            mainCtx.beginPath();
            mainCtx.arc(myPosX2, myPosY2, 5, 0, window.wsStuffs.PI, false);
            mainCtx.closePath();
            mainCtx.fill();
          } else console.log("3d54")
        }
      }
    ...
    function drawGame() {
      ...
      var drawList = (wHandle.globalPreference.TAB_ACTIVE ? cells2 : cells).list.slice(0).sort(cellSort);
    ...
    function cameraUpdate() {
      var myCells = [];
      var cl = wHandle.globalPreference.TAB_ACTIVE ? cells2 : cells;
    ...
    Cell.prototype = {
      ...
      destroy: function (killerId) {
        if (this.multi)
          if (this.id in cells2.byId) delete cells2.byId[this.id];
            else if (this.id in cells.byId) delete cells.byId[this.id];
        if (this.multi && cells2.mine.remove(this.id) && cells2.mine.length === 0) {
          wHandle.globalPreference.TAB_ACTIVE = false;
        } else if (!this.multi && cells.mine.remove(this.id) && cells.mine.length === 0) {
          wHandle.globalPreference.TAB_ACTIVE = false;
          cleanupObject(cells);
          cells.mine = [];
          cells.byId = {};
      ...
      setName: function (value) {
        if (typeof value === "string" && value.match(/\[.*\]/g)) {
          let accountDetails = wHandle.parseName(value);
          if (
            wHandle.account &&
      ...
      drawShape: function (ctx) {
        ...
        if (this.jagged) {
          ...
          ctx.lineWidth = "20";
          ctx.arc(this.x, this.y, this.s, 0, window.wsStuffs.PI, false);
        } else {
        ...
        if (settings.showSkins && this.skin) {
          ...
          if (skin && skin.complete && skin.width && skin.height) {
            ...
            let sc = sScaled * 2.5;
            let skinActivated = wHandle.settingProperty("GENERALE", "SHOW_SKIN_CELL");
            ...
            let lineActivated = wHandle.settingProperty("GENERALE", "LINE_MOUSE");

            if (this.pidPlayer === wHandle.globalPreference.PID_PLAYER) {
              scaleBack(ctx);
              let MuoaActivated = wHandle.settingProperty("GENERALE", "MUOA_CIRCLE");
              ...
              if (
                ...
                MaouCircle &&
                MaouCircle.canvas &&
                ((!this.name.match(/(\%\^)+$/) && wHandle.globalPreference.TAB_ACTIVE === false) ||
                  (this.name.match(/(\%\^)+$/) && wHandle.globalPreference.TAB_ACTIVE === true))
              ) {
                ctx.drawImage(
                  MaouCircle.canvas,
                  this.x * cameraZ - sc + sc / 2,
                  this.y * cameraZ - sc + sc / 2,
                  sc,
                  sc,
                );
              }
      ...
      drawText: function (ctx) {
        ...
        let nameActivated = wHandle.settingProperty("GENERALE", "SHOW_NAME_CELL");
        let massActivated = wHandle.settingProperty("GENERALE", "MASS_CELL");
        let cl = wHandle.globalPreference.TAB_ACTIVE ? cells2 : cells;
          ...
          if (this.name) {
            if (this.tag) {
              drawText(
                ...
                false,
                this.x,
                this.y - (Math.max(this.s / 4.5, this.nameSize / 1.5) + 10),
                this.nameSize / 2,
                this.drawNameSize / 2,
                this.tag,
              );
          ...
          if (nameActivated !== false) {
            if (this.tag) {
              drawText(
                ...
                false,
                this.x,
                this.y - Math.max(this.s / 4.5, this.nameSize / 1.5),
                this.nameSize / 2,
                this.drawNameSize / 2,
                this.tag,
              );
    ...
    function drawTextOnto(canvas, ctx, text, size) {
      ...
      let nameColored = wHandle.settingProperty("GENERALE", "COLOR_NAME_CELL");
      let nameStrokeColored = wHandle.settingProperty("GENERALE", "OUTER_COLOR_TEXT");
    ...
    wHandle.spectate = function (a) {
      wsSend(window.wsStuffs.cache[1]);
      stats.maxScore = 0;
      hideESCOverlay();
    };
    wHandle.play = function (a) {
      sendPlay(a);
      if (wHandle.closePanel) wHandle.closePanel();
      hideESCOverlay();
    };
