export const MainOut = function (wHandle, wjQuery, loaded) {
  ...
  var wsUrl = null,
    ...
    SEND2_254 = new Uint8Array([254, 6, 0, 0, 0]),
    SEND2_255 = new Uint8Array([255, 1, 0, 0, 0]),
    UINT8_CACHE = {
      1: new Uint8Array([1]),
  ...
  function wsInit(url, wsOnly = false) {
    if (ws) {
    ...
    if (!wsOnly) {
      ...
      ws2.onopen = () => {
        wsSend(SEND2_254, true);
        wsSend(SEND2_255, true);
        connected = true;
        ...
        switch (packetId) {
          ...
          case 0x40: // set border
            if (window.globalPreference.TAB_ACTIVE) {
              ...
              if (!mapCenterSet) {
                mapCenterSet = true;
                cameraX2 = targetX2 = border.centerX;
                cameraY2 = targetY2 = border.centerY;
                cameraZ2 = targetZ2 = 1;
  ...
  function sendMouseMove(x, y, isWS2) {
    ...
    if (isWS2) wsSend(writer, true);
      else wsSend(writer, false);
  }
  ...
  function gameReset() {
    ...
    cameraZ = targetZ = 1;
    cameraX2 = cameraY2 = targetX2 = targetY2 = 0;
    cameraZ2 = targetZ2 = 1;
    mapCenterSet = false;
  }
  ...
  function drawLeaderboard() {
    let miniLeadersActivated = window.settingProperty("LEADERS", "LEADERBOARD");
    if (leaderboard.type === NaN || !miniLeadersActivated) return (leaderboard.visible = false);
  ...
  function drawBorders() {
    ...
    let miniGridsActivated = window.settingProperty("GENERALE", "GRIDS");
    if (miniGridsActivated) {
  ...
  function drawMinimap() {
    let miniMapActivated = window.settingProperty("MINIMAP", "MINIMAP_ENABLED");
    if (border.centerX !== 0 || border.centerY !== 0 || !miniMapActivated) return;
    mainCtx.save();
    var targetSize = 150;
  ...
  function cameraUpdate() {
    ...
    if (myCells.length > 0) {
      ...
      if (window.globalPreference.TAB_ACTIVE) {
        targetX2 = x / l;
        targetY2 = y / l;
        targetZ2 = Math.pow(Math.min(64 / s, 1), 0.4);
        cameraX2 += (targetX2 - cameraX2) / 2;
        cameraY2 += (targetY2 - cameraY2) / 2;
      }
      stats.score = score;
      ...
      if (window.globalPreference.TAB_ACTIVE) {
        cameraX2 += (targetX2 - cameraX2) / 20;
        cameraY2 += (targetY2 - cameraY2) / 20;
      }
    }
    if (window.globalPreference.TAB_ACTIVE) {
      cameraZ2 += (targetZ2 * viewMult * mouseZ - cameraZ2) / 9;
    }
  ...
  function init() {
    ...
    setInterval(function () {
      ...
      cameraZ += (targetZ * viewMult * mouseZ - cameraZ) / 9;
      cameraZ2 += (targetZ2 * viewMult * mouseZ - cameraZ2) / 9;
      let c = window.globalPreference.TAB_ACTIVE, mX,mY;
      if (c) {
        mX = (mouseX - mainCanvas.width / 2) / (cameraZ2) + (cameraX2),
        mY = (mouseY - mainCanvas.height / 2) / (cameraZ2) + (cameraY2);
        sendMouseMove(mX, mY, true);
      }
      mX = (mouseX - mainCanvas.width / 2) / (cameraZ) +  cameraX;
      mY = (mouseY - mainCanvas.height / 2) /  cameraZ +  cameraY;
      sendMouseMove(mX, mY, false);
    }, 40);
